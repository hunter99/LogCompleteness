...
/**
 * The second version of our approach to estimate the informative completeness.
 * <P> N>=W^2/(e^2(1-k))
 * 
 * @author hedong
 *
...
public abstract class AbstractCompletenessEstimatorV2 extends AbstractComletenessEstimator implements Estimator {
	public AbstractCompletenessEstimatorV2(String name,double e, double c) {
		super(name,e,c);
	}
	public void calculate(double epsilone, double confidence, long numOfObservedUnits, long loglength, StatRes res) {

		double divisor = epsilon * epsilon * (1 - confidence);
		double dividend = numOfObservedUnits * numOfObservedUnits;
		if(divisor == 0){
		...
		double divisor2 = Math.sqrt(loglength * (1 - confidence));
		double dividend2 = numOfObservedUnits;
		if(divisor2 == 0){
			res.setCoverageProbability(-1);
			res.setNewClassProbability(-1);
			return;
